...
- **The goal:** lift one accountant from 30 to 60 customers (and stay on the curve to 100+). The only way to do that is to stop touching the confident bulk by hand and spend attention only on the few that are genuinely uncertain. Two rates stay distinct throughout: the agent's per-decision *categorisation* confidence (the brief's "confident in categorising" beat, evidenced by 0.95 categorisation accuracy) versus the stricter all-in *auto-post* gate (~84%) that also needs the match and a clean guard.
- **The constraint that defines the product:** this is a money system. A wrong post flows into VAT and filings. The number that kills trust is **false confidence**: the agent was sure and wrong. Our target: **zero on reconciliation and on any material entry** (material = |amount| ≥ €1,000, an accountant-owned, per-customer calibration knob), **with a small measured residual on immaterial categorisation** — measured every run, never faked to a flat zero.
...
Every transaction runs the same fixed, auditable path — topology, the tool boundary, and where the human sits are on one page in `ARCHITECTURE.md`.
...
3. **Guard (code, strict):** re-check the proposal against hard facts — account exists, the amount sums exactly, no same-amount sibling collision, no revenue on a settled invoice, the document not already claimed, direction right, the vendor→cost-account correction honoured, any material missing document flagged, no duplicate-payment fingerprint (the full list is in `ARCHITECTURE.md`). Any failure downgrades the route; code can only downgrade, never rewrite the agent's choice.
...
- **What the guard backstops — and what it does not.** It catches fact contradictions (account-not-in-chart, amount mismatch, the same-amount sibling collision that produced the old confidently-wrong posts, revenue on a settled invoice, a document claimed twice, wrong direction, a duplicate-payment fingerprint) and the **vendor→cost-account** correction class. Conventions that hinge on judgement (asset vs supplies, owner-draw) and match re-points are **not** guard-enforced — they are held by the confidence gate plus the eval, stated honestly rather than papered over.
- **Never confidently wrong is measured, not asserted.** The eval counts auto-posted-and-wrong per run, split by task. **Reconciliation and material entries: zero, every run** (material = |amount| ≥ €1,000, an accountant-owned, per-customer calibration knob). Categorisation false-confidence is **1–3 (mean 2) across three runs, all immaterial (≤€210, well under that line)** — and inspecting them, they are ground-truth-weakness or judgment-calls, not clear model errors. Held by grounding + the guard + calibration + the human, and reported every run rather than claimed.
...
- **The brief's anomaly trio, each with a home.** Missing-counterpart → request the document from the entrepreneur (when material); fraud → a suspicious-vendor or unusual-amount flag raised only on a concrete single-transaction signal; miscategorisation is deliberately *not* a separate flag — it surfaces as low account-confidence → review, where every uncertain categorisation already lands. Duplicates are detected across the full set (above), never guessed per transaction.
...
- **Three actions:** **accept** (post it), **correct** (the dropdown fix that triggers the learning loop and reports "N similar re-run"), **explain** (a deterministic, code-built narration of the decision — not a second LLM call; a real follow-up model call is a small next step, not claimed today).
- **Anomaly card → what the accountant clicks next:** request the missing bill, or confirm the duplicate — the one flagged transaction routed to its explicit next step, not just surfaced.
...
## Slide 9 — The demo, three beats

The live screen-share maps 1:1 to the brief's three demo requirements.

- **Beat 1 — the auto-posted tab (the confident bulk).** Open `/posted`: the bulk the agent is confident *categorising*, each row with its one-line reasoning visible. Cite categorisation accuracy **0.95** (k=3 0.93–0.96, over all 100); the agent rates account-confidence high / medium / low independently, so this beat is about categorisation confidence, not the stricter all-in auto-post share.
- **Beat 2 — the review queue (the few uncertain).** Open the queue: each deferred card shows the choice, the grounded context it reasoned over (computed facts, candidate documents, any applicable correction), and the two confidences — so the accountant sees *why* it is uncertain.
- **Beat 3 — the anomaly card (the one flagged).** The pinned anomaly is the duplicate payment: its reasoning, and the exact next click — confirm the duplicate (a missing-document anomaly gets *request the bill* instead).

---

## Slide 10 — The eval numbers (what is produced, and how)
...
- **Auto-post rate is not the categorisation-confidence rate.** Auto-post (~84% across runs, 85/100 in the committed run) is the stricter all-in gate — account-confidence high AND match-confidence high AND a clean guard — so it sits below the share the agent is confident *categorising* alone, which it rates high / medium / low independently. The brief's "confident in categorising" beat is carried by the categorisation accuracy above (0.93–0.96), not by the all-in auto-post number.
...
- **Deferral quality on the labelled set:** ground truth is **81 auto / 16 review / 1 anomaly** (the duplicate) / **2 request-document** (the two material missing-document cases). The eval scores whether the engine catches that 1 anomaly with no false positives (it does, every run) and how it routes the requests: it catches **both** material missing-document cases (request-document recall **1.0**) but over-defers one (T083), so request-document precision is **0.67** (3 predicted, 2 correct) — left and disclosed, because a threshold fix would only data-fit the seeds.
...
## Slide 11 — Tooling: the five choices, each defended

- **Model:** one interpretation step, so one strong frontier model serves it (Claude Sonnet 4.6, or Gemini 2.5 Pro behind the same interface). **Defence:** the precision bar is high (errors hit VAT and filings), so capability beats per-call cost; a cheap-model-on-the-bulk, strong-on-the-tail cascade is a gated option, kept off until the eval shows a cheaper model (Flash) holds the accuracy on the confident bulk.
- **Orchestration:** our own deterministic pipeline, with pydantic-ai wrapping the one structured-output call (the committed run swaps in a `claude -p` backend behind the same `AgentProtocol`). **No agent framework.** **Defence:** the per-transaction flow is fixed (one LLM step), not an open-ended loop; an agentic loop would mean 2+ LLM calls per transaction (across 100+ customers × ~100 txns each) for **no correctness gain**, because the output is a structured decision, not rendered prose the model must re-read. Tool-calling is reserved for the entrepreneur side, where it is genuinely needed.
- **Retrieval and context:** corrections pulled into the agent's evidence — general conventions always, vendor-specific ones keyed by **canonical vendor, not embeddings** — with the cost-account guard behind that class. **Defence:** exact keying is the deliberate anti-RAG choice; in a money system a fix must not silently over-generalise onto unrelated transactions, which a fuzzy semantic match risks. All five seeded corrections fit in the prompt today; vendor filtering (then retrieval) is the scale step as the store grows, and a missed vendor→cost-account fix still cannot cause a wrong post because the guard enforces it.
- **Evals:** the harness above, scoring categorization, reconciliation, false confidence (k=3 for the non-determinism), per-outcome gates, and the cold-vs-corrected lift. **Blocked responses (the brief's third eval pillar):** schema/parse and model errors are caught by retries and counted as errored decisions (committed run: 0 of 100 cold, 0 of 100 warm), and the share blocked from auto-post — **15 of 100** (11 review, 1 anomaly, 3 request-document) — is scored by the per-outcome gates, so over- and under-blocking are both visible. **Defence:** false confidence is the number that kills trust, so we measure it per run as a range, not a claim; the lift is the concrete proof that learning works.
...
## Slide 12 — Capacity: minutes saved per customer per month
...
- **From 4.8× on this step to 2× overall (a stated assumption, pending telemetry):** the 4.8× is the cut on the categorisation + reconciliation workflow alone. The 30→60 headline is ~2× because that workflow is only part of an accountant's per-customer time — assume it is the larger share, on the order of two-thirds, with advisory, onboarding, and VAT-filing review out of scope here and unchanged. That share is what reconciles a 4.8× workflow cut with a ~2× headcount multiple; it widens toward 100+ as the auto-rate climbs and more steps come into scope.
- **Caveat:** `B`, `s`, `R`, `V` and the categorisation+reconciliation share of total accountant time are assumptions pending real telemetry; only `a` is measured (and on real data day-one `a` is lower, rising as corrections accumulate). The durable claims are the formula and "auto-rate dominates minutes, ranking holds trust."

---

## Slide 13 — Outcomes, assumptions, timelines

**The outcomes we claim and how we hold them.**

- **Capacity 30 to 60:** the accountant only touches the uncertain few and the anomalies; the confident bulk auto-posts; the queue ranking sends attention where a mistake is both likely and costly (Slide 12).
- **False confidence held low:** zero on reconciliation and material entries; a small, measured, immaterial residual on categorisation (1–3 of 100, k=3), held and measured every run (Slide 6).
- **Learning:** one correction moves the next same-vendor ones (7 eligible in the committed run), re-run live; the mechanism is unit-tested, and the cold→warm lift measures it at **about +0.5** (k=3; the committed run is +0.57 net, not monotonic — T036 regressed) on the eligible rows.
